...
const WACarousel = ({ onRotationComplete }) => {
  ...
  const fetchWatchlistTickers = async () => {
    try {
      ...
      const tickers = data.watchlist.map(item => item.symbol);
      setWatchlistTickers(tickers);
      return tickers;
    } catch (err) {
      console.error('Error fetching watchlist:', err);
      setError('Failed to load watchlist');
      // Fallback to default tickers if watchlist fails
      const fallbackTickers = ['AAPL', 'GOOGL', 'MSFT', 'TSLA', 'NVDA'];
  ...
  const fetchTickerData = async (ticker, timeView) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/stock/${ticker}?period=${period}&interval=${interval}`);
  ...
  const preloadAllData = async (tickers) => {
    ...
    try {
      ...
      // Filter out any undefined/null entries and ensure we have all time views
      const cleanedTickerDataMap = new Map();
      tickerDataMap.forEach((dataArray, ticker) => {
        const cleanedArray = dataArray.filter(item => item !== undefined && item !== null);
        if (cleanedArray.length > 0) {
          cleanedTickerDataMap.set(ticker, cleanedArray);
        }
      });
      
      setAllTickerData(cleanedTickerDataMap);
      console.log('All watchlist ticker data preloaded successfully');
      console.log('Data structure:', Array.from(cleanedTickerDataMap.entries()).map(([ticker, data]) => ({
        ticker,
        timeViews: data.map(d => d.timeView)
      })));
    } catch (error) {
  ...
  useEffect(() => {
    // Fetch watchlist tickers first, then preload data
    const initializeData = async () => {
      const tickers = await fetchWatchlistTickers();
      if (tickers.length > 0) {
        await preloadAllData(tickers);
      }
    };
    ...
    const dataRefreshInterval = setInterval(() => {
      console.log('Refreshing watchlist ticker data in background...');
      if (watchlistTickers.length > 0) {
        preloadAllData(watchlistTickers);
      }
    }, CACHE_DURATION);
  ...
  useEffect(() => {
    ...
    if (!isPreloading && allTickerData.size > 0 && watchlistTickers.length > 0) {
      ...
      <div style={{ 
        ...
        <div style={{
          ...
          <div style={{
            ...
            {Math.round(preloadProgress)}% complete ({watchlistTickers.length} tickers × {timeViews.length} time views)
          </div>
          {error && (
            <div style={{
              color: '#dc2626',
      ...
      <div style={{ 
        ...
        <div style={{
          ...
          <div style={{
            ...
            Add some symbols to your watchlist to start the carousel
          </div>
        </div>
      </div>
    );
